...
fn apply_direct(op: &DdlOperation, schema: &Schema, engine: &Arc<StorageEngine>) -> Result<()> {
    match op {
        ...
        DdlOperation::CreateIndex(idx) => {
            ...
            // Build it here too. Recording the index and not having it is what
            // made a node refuse an index its own schema listed (t_1f2741a0).
            if let Some(table) = schema
                .snapshot()
                .tables
                .get(&(idx.keyspace.clone(), idx.table.clone()))
            {
                crate::index_wiring::wire_index_into_engine(engine, table, idx);
            }
            SystemTableWriter::new(Arc::clone(engine))
                .apply(
