//! t_3fc6be3c — the multi-replica coordinated projected paged-scan HANG.
//!
//! A multi-page projected scan on a multi-replica (RF>1) cluster used to HANG.
//! The single-node (`expected_done == 0`) fan-out that the earlier
//! `range_scan_projected_partition_limit` test exercises CANNOT reproduce it —
//! there are no remote producers to leak. This test drives the REAL
//! multi-replica fan-out (`expected_done == 2`) over an in-process 3-node
//! loopback so page 2+ genuinely resumes the coordinated scatter-gather.
//!
//! Root cause: when the CQL paging collector fills a page and DROPS the merged
//! stream, the coordinator-side merge/forwarder tasks die but the REMOTE replica
//! producers (`handle_stream_request_with_cancel`) were never told to stop —
//! `RangeReadStreamCancel` was wired on the receiver but never SENT by the
//! coordinator. Each abandoned producer keeps scanning the whole table and firing
//! chunk frames back on `Lane::Bulk`; `pages × replicas` of them pile up and
//! starve the connection's heartbeat traffic until the driver declares it
//! defunct.
//!
//! Repro harness:
//!  * 3 real `RpcServer`s over 127.0.0.1 loopback: node1 = coordinator,
//!    node2 + node3 = replicas. RF=3, CL=ALL → 2 remotes → `expected_done == 2`.
//!  * Each replica serves a `RangeReadStreamRequestHandler` whose reader PARKS
//!    forever after emitting a bounded prefix of partitions (models an in-flight,
//!    not-yet-exhausted scan). A live-producer counter increments when the reader
//!    task starts and decrements when it is dropped/cancelled.
//!  * The coordinator registers a `StreamFrameRouter` so replica chunk frames
//!    reach its per-request receivers.
//!
//! RED (before the fix): abandoning a page leaves the parked replica producers
//! alive — the live-producer counter never returns to 0, so the wait times out
//! and the test FAILS (a hang deterministically fails, it does not wedge CI).
//!
//! GREEN (after the fix): dropping the merged stream fires
//! `RangeReadStreamCancel` to every replica; each parked producer observes its
//! `CancellationToken`, returns, and the live-producer counter drops to 0.
...
fn multi_replica_paged_projected_scan_cancels_abandoned_producers() {
    ...
    rt.block_on(async move {
        ...
        registry.register(MsgType::RangeReadStreamDone, frame_router);
...
async fn spawn_storage_replica(
    ...
    let sink_factory = Arc::new(PeerManagerSinkFactory::new(back.clone()));
    // Small chunk_size so multiple chunks flow per page.
...
/// full keyset with NO gaps and NO duplicates, and the loop must TERMINATE (a
/// hard timeout guards every page and the whole loop so a regression FAILS
...
fn multi_replica_paged_projected_scan_returns_all_rows_across_pages() {
    ...
    rt.block_on(async move {
        ...
        let whole = tokio::time::timeout(Duration::from_secs(30), async {
            loop {
                pages += 1;
                ...
                while page_keys.len() < PAGE {
                    let Some(item) = tokio::time::timeout(Duration::from_secs(15), stream.next())
                        .await
                        .expect("a page pull must not hang")
                    else {
                ...
                // like the CQL collector between pages. With the fix this cancels
                // the remote producers cleanly so the NEXT page does not deadlock.
                drop(stream);
