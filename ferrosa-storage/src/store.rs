...
impl<F: FlushTarget> TableStore<F> {
    ...
    /// Fail-open full-text search over the merged table contents.
    ///
    /// Normal `fts_match` reads persisted FTI sidecars plus active/flushing
    /// memtables. During sidecar rebuild windows, after operator repair, or if
    /// a sidecar artifact is missing/corrupt, durable flushed rows must not
    /// become invisible. This bounded fallback builds a transient FTI from the
    /// existing range-read path and evaluates the same parsed query semantics.
    pub fn fulltext_table_scan_search(
        &self,
        index_name: &str,
        query: &str,
    ) -> ferrosa_common::Result<Vec<(Vec<u8>, f64)>> {
        ...
        let partitions = self.read_range(None, None, RANGE_READ_MATERIALIZATION_CAP)?;
        let mut builder = FullTextIndexBuilder::new();
        for partition in partitions {
            let pk_bytes = partition.key.key.as_bytes().to_vec();
            let mut text = String::new();
        ...
        let bytes = serialize_fti(&fti).map_err(|e| {
            ferrosa_common::Error::InvalidFormat(format!("fts_match table-scan index error: {e}"))
        })?;
        let reader = FullTextIndexReader::open(bytes).map_err(|e| {
            ferrosa_common::Error::InvalidFormat(format!("fts_match table-scan index error: {e}"))
        })?;
