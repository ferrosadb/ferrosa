...
| **Bootstrap phase runner** | `cluster-formation-architecture.md` §Bootstrap; `dsm-cluster-formation.md` A1/A1a–A1m | **Full** — canonical `DeliverInvites`, `EstablishPools`, `CreateRaft`, `WaitLeader`, `ReplaySchema`, `BootstrapStream`, `Promote`, and `DrainQueue` phases described with sequence diagram |
