...
async fn read_v(s: &Session, table: &str, cl: Consistency) -> String {
    ...
    match s.query_unpaged(st, &[]).await {
        Ok(res) => match res.into_rows_result().ok().and_then(|rr| {
            rr.rows::<Row>()
                .ok()
...
async fn main() -> Result<()> {
    let contact = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "127.0.0.1:9042".into());
    let s = connect(&contact).await?;

    s.query_unpaged("DROP KEYSPACE IF EXISTS probe", &[])
        .await
        .ok();
    s.query_unpaged(
    ...
    .await?;
    s.query_unpaged("CREATE TABLE probe.s (k int PRIMARY KEY, v int)", &[])
        .await?;
    s.query_unpaged("CREATE TABLE probe.l (k int PRIMARY KEY, v list<int>)", &[])
        .await?;
    s.query_unpaged("CREATE TABLE probe.sn (k int PRIMARY KEY, v int)", &[])
        .await?;
    s.query_unpaged(
        "CREATE TABLE probe.ln (k int PRIMARY KEY, v list<int>)",
        &[],
    )
    .await?;
    ...
    println!(
        "[Accord txn] SCALAR  immediate: serial={}  quorum={}",
    ...
    println!(
        "[Accord txn] LIST    immediate: serial={}  quorum={}",
    ...
    s.query_unpaged("UPDATE probe.sn SET v = 99 WHERE k = 1".to_string(), &[])
        .await?;
    s.query_unpaged(
        "UPDATE probe.ln SET v = v + [5] WHERE k = 1".to_string(),
        &[],
    )
    .await?;
    println!(
        "[plain]      SCALAR  immediate: quorum={}",
        read_v(&s, "sn", Consistency::Quorum).await
    );
    println!(
        "[plain]      LIST    immediate: quorum={}",
        read_v(&s, "ln", Consistency::Quorum).await
    );

    // 4. Re-read the Accord writes after a delay (does the collection appear later?).
    tokio::time::sleep(std::time::Duration::from_secs(5)).await;
    println!(
        "[Accord txn] SCALAR  after 5s:  quorum={}",
        read_v(&s, "s", Consistency::Quorum).await
    );
    println!(
        "[Accord txn] LIST    after 5s:  quorum={}",
        read_v(&s, "l", Consistency::Quorum).await
    );
