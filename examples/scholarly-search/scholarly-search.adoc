...
|===
| Lens | Answers | How
| *Keyword* (BM25) | "papers that literally say _nearest neighbor_ and _memory_" | full-text index + `fts_match()`
...
== Lens 1 -- Keyword (BM25 full text)

[source,sql]
----
CREATE INDEX paper_fts ON paper (abstract) USING 'fulltext';

SELECT paper_id, title FROM paper
  WHERE abstract = fts_match('nearest neighbor AND memory');
----

`fts_match()` runs a BM25-scored boolean query over the inverted index. It is
precise when you know the words -- and blind to synonyms or paraphrases. That's
what the next lens fixes.
...
No single lens is "the answer." A great search _composes_ them. Because BM25,
vector, and phonetic each return a *ranked list*, we fuse them with
