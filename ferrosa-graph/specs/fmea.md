...
| G-7 | **Reconciler swallows read errors** — scan failures are skipped rather than surfaced | A transient storage error makes a pass silently under-repair (or skip orphan detection) while reporting success | 5 | 3 | 5 | 75 | **Partially mitigated (t_bc5f0e6f).** Both scans are now streamed and every failure — opening the scan, and failing mid-stream — is logged with the table/keyspace and how far it got, so a degraded pass is visible. The pass is still best-effort by design (it is a periodic safety net; the next pass retries). **Remaining gap:** the failures are not reflected in `ReconcileMetrics`, so a caller reading the metrics alone still cannot tell a complete pass from a partial one. |
| G-9 | **Whole-table materialization in the read path** — `WritePath::range_read` drains a full-table scan into `Vec<Partition>`, and eleven graph call sites used it (anchor scan, per-hop edge scan, var-length seed + fallback, the three find-* fallbacks, both reconcile scans) | A single unanchored `MATCH`, or a background reconcile pass, pulls a tenant-sized table into the heap; on a node with a memory cap that is an OOM kill, not a slow query | 9 | 4 | 3 | 108 | **Mitigated (t_bc5f0e6f, t_250fa355).** Every site pulls one partition at a time via `range_read_stream_all`; the three find-* fallbacks stop at their first match. Guarded by `ferrosa-graph/tests/graph_range_read_memory_bound.rs` (peak heap must not scale with table size) and by the `p0-oom-audit` static gate, whose ferrosa-graph exemptions are deleted. **Remaining:** phase-A `Expand` still accumulates one `ExpandState` per surviving anchor row, so a query whose filters select most of a huge table still holds a query-sized frontier — bounded by the answer, not by the table, but not yet streamed end to end (t_4ce82a3e). |
