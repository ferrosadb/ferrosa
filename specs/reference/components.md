...
- **Design**: UDFs are either uploaded as precompiled WASM binaries via `CREATE FUNCTION ... LANGUAGE wasm AS <hex>` (or `AS FILE`/`AS URL`), or written as inline source via `CREATE FUNCTION ... LANGUAGE assemblyscript AS '<source>'` and compiled at definition time. The executor compiles and caches modules with real Wasmtime `Component` instantiation, enforcing per-invocation resource limits via Wasmtime fuel and epoch interruption. CREATE/DROP FUNCTION and CREATE/DROP AGGREGATE are parsed by the CQL parser, routed through `DdlPath` with permission checks, and replicated via `DdlOperation`/`RaftCommand`. `FunctionMetadata` and `AggregateMetadata` are stored in the schema registry with `system_schema.functions` and `system_schema.aggregates` virtual tables. Rust is the preferred authoring language for precompiled UDFs; any language targeting the WASM Component Model works.
  - **Inline AssemblyScript** (`ferrosa-udf`, behind the `asc-udf` feature; `ferrosa_udf::asc` + `ferrosa_udf::component`): runs the `asc` + `binaryen` toolchain inside QuickJS with a wasmtime-backed `WebAssembly` shim, compiling AS source to a core module, then componentizing it to the `udf` WIT world. Supports numeric (`int`/`bigint`/`float`/`double`/`smallint`/`tinyint`), `text`/`ascii` (AS `string`), and `blob` (AS `Uint8Array`) argument and return types. The toolchain bundle is located via `FERROSA_ASC_BUNDLE`.
...
- **Modules**:
  ...
  - `repair/executor.rs` — `RepairStore` trait, `LocalRepairExecutor` (production initiator), `StorageEngineRepairStore` (local side wrapping `Arc<StorageEngine>`), `InMemoryRepairStore` (test side); drives the Merkle-then-stream session with chunked Fetch/Apply (`REPAIR_FETCH_CHUNK_PARTITIONS=64`, `REPAIR_FETCH_CHUNK_BYTES=32MiB`, `REPAIR_APPLY_CHUNK_PARTITIONS=64`, `MERGED_SPAN_MAX_LEAVES=8`)
  - `repair/rpc.rs` — wire protocol: `RepairMerkleRequest/Response`, `RepairFetchRequest/Response` (with `cursor` + `limit` + `max_bytes` for chunked iteration), `RepairApplyRequest/Response`. Handlers: `RepairMerkleHandler`, `RepairFetchHandler`, `RepairApplyHandler`. Client: `RemoteRepairStore` (RPC over `PeerManager`, all three RPCs sent on `Lane::Bulk`).
