...
Ferrosa is a Rust reimplementation of an Apache-Cassandra-shaped database with S3 as the durable substrate and local NVMe as a cache. It is a Cargo workspace of 17 crates with a strictly acyclic dependency graph; the `ferrosa` binary composes them into a single node that speaks CQL, Cypher (Bolt + HTTP), SPARQL, and a Prometheus/JSON observability surface on one process.
...
**Binary.** [`ferrosa`](../ferrosa/) is the composition root. Startup order in [`ferrosa/src/main.rs`](../ferrosa/src/main.rs) is: tracing → host_id → `StorageEngine::new` → `Schema::new` → `ModeController::new` → `PeerManager` → RPC server on :7000 → CQL on :9042 → optional graph HTTP :7474 + Bolt :7687 → SPARQL :8080 → web console + cluster REST API on :9090.
...
Three independent entry points, each with its own auth stack, plus a single kill switch.
