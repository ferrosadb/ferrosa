...
| S1.5 | Add 4 commit log entry types: `AccordPreAccepted`, `AccordAccepted`, `AccordCommitted`, `AccordApplied` to ferrosa-storage | M | Serialize/deserialize roundtrip preserves all fields. Entry size checked against segment size limit (return `Err`, not panic, per UDF/UDA branch fix). | `accord_commitlog_roundtrip`, `accord_entry_size_validation` | Spec §10, FM5, FM14 |
...
| # | Task | Size | Success Criteria | Tests | Source |
|---|------|------|-----------------|-------|--------|
| S3.1 | Implement `AccordStateMachine` in ferrosa-cluster/accord/: PreAccept, Accept, Commit, Execute, Apply handlers with all phase transitions | XL | State machine transitions match spec §4.1-4.5. Each handler persists commit log entry before replying. Idempotent on duplicate messages. | `state_machine_phase_transitions`, `state_machine_idempotency` | Spec §4, FM5 |
