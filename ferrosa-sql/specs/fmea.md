...
| SQL-12 | **The final `QueryResult.rows` is still a `Vec`.** Every operator feeding it streams or spills, but `execute` drains the pipeline into memory because `ferrosa-postgres::render_result` builds all `DataRow` messages before writing any. | A result set larger than RAM still OOMs, at the front-end boundary rather than inside an operator | 7 | 3 | 3 | 63 | **Open, narrowed and documented** on `QueryResult`. The fix is front-end work: stream `DataRow`s to the wire, then hand `execute` a row stream. Detection is now easy — it is the one remaining buffer. |
